...
export class InformeProyectosComponent implements OnInit {
  ...
  if (selectorCiudad)
  {
    selectorCiudad.addEventListener('change', e => { //me permite ver cuando estoy cambiando de opcion
      const list = e.target;
  ...
  busquedaPais(id:number){
    this.servicioPaises.obtenerCiudadesPais(id).subscribe(ciudades=>{
      ciudades.forEach(ciudad=>{
        if(ciudad.codigo)
        this.servicioCiudad.obtenerProyectoCiudad(ciudad.codigo).subscribe((proyectos)=>{
          proyectos.forEach(proyecto=>{
            if(proyecto.codigo)
            this.servicioProyecto.obtenerBloquesProyecto(proyecto.codigo).subscribe(bloques=>{
              bloques.forEach(bloque=>{
                if(bloque.codigo)
                this.servicioBloque.obtenerBloqueInmueble(bloque.codigo).subscribe(inmuebles=>{
                  inmuebles.forEach(inmueble=>{
                    if(inmueble.codigo)
                    this.servicioSolicitudes.obtenerSolicitudesTotalesInmueble(inmueble.codigo).subscribe(solicitudes=>{
                      ...
                      this.barChartData = [
                        { data: [this.vendidos], label: 'Inmuebles vendidos' },
                        { data: [this.noVendidos-this.vendidos], label: 'Inmuebles sin Vender' }
                      ];
                      console.log(this.vendidos+' '+ this.noVendidos);
                    });
