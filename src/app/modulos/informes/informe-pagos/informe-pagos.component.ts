...
export class InformePagosComponent implements OnInit {
  ...
  llenarClientes() {
    ...
    this.servicioClientes.obtenerClientes().subscribe((clientes) => {
      ...
      this.clientes?.forEach((cliente) => {
        ...
        let nombreCliente =
          cliente.Nombre + ', con Documento: ' + cliente.Documento;
  ...
  generarGrafica(idCliente: number) {
    ...
    this.servicioSolicitud
      .obtenerSolicitudesCliente(idCliente)
      .subscribe((inmuebles) => {
        inmuebles.forEach((element) => {
          this.total = 0;
          if (element.ofertaEconomica) this.total = element.ofertaEconomica;
          if (element.codigoInmueble)
            this.servicioInmueble
              .obtenerInmueble(element.codigoInmueble)
              .subscribe((inmueble) => {
                //this.inmueble=inmueble.identificador;
                this.inmuebles.push(inmueble);
                if (element.codigo) this.llenarInmuebles(element.codigo);
              });
        });
      });
  }
    ...
    if (selectorInmueble) {
      selectorInmueble.addEventListener('change', (e) => {
        ...
        this.servicioSolicitud.obtenerSolicitud(id).subscribe((elemento) => {
          this.total = 0;
          if (elemento.ofertaEconomica) this.total = elemento.ofertaEconomica;
